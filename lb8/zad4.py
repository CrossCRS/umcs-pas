...
def main():
    ...
    response = send_imap(sock, "SEARCH UNSEEN")

    if response[1].find(b"OK") == -1:
        sock.close()
        exit(-1)

    msg_ids = response[0].decode().replace("\r\n", "").split(" ")[2:]

    for msg in msg_ids:
        # FETCH automatycznie oznacza rowniez wiadomosc jako SEEN
        response = send_imap(sock, f"FETCH {msg} BODY[TEXT]", getall=True)
