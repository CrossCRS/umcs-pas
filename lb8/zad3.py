...
def main():
    ...
    for line in response:
        if line.startswith(b"*"):
            folders.append(line.split(b" ")[-1].replace(b"\r\n", b"").decode())

    msg_count = 0
    for folder in folders:
        response = send_imap(sock, f"STATUS {folder} (MESSAGES)")
        count = re.findall(r"\(MESSAGES (\d+)\)", response[0].decode())[0]
        msg_count += int(count)
